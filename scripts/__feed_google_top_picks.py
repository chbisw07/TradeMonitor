...
def _args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--create-entry-intents",
        action="store_true",
        help="after Intake, create canonical PAPER EntryIntents for verified Top Picks statuses",
    )
            ...
            payload.update(
                {
                    ...
                    "expiry": (
                        re.search(r"\b(20\d{2}-\d{2}-\d{2})\b", str(suggested or "")).group(1)
                        if re.search(r"\b(20\d{2}-\d{2}-\d{2})\b", str(suggested or ""))
                        else ""
                    ),
...
def main() -> int:
    ...
    if args.create_entry_intents:
        print("\nEntryIntent translation preview:")
        for item, _state_key in selected:
            translation = translate_top_pick_to_entry(item.observation)
            label = "ARM" if translation.arm else "SKIP"
            print(f"  {label:<4} R{item.row_number} {item.observation.intent.underlying:<12} {translation.reason}")
    ...
    try:
        ...
        for item, state_key in selected:
            ...
            if args.create_entry_intents:
                translation = translate_top_pick_to_entry(item.observation)
                if translation.arm and translation.kwargs is not None:
                    entry = manager.create_entry_intent(episode=result.episode, **dict(translation.kwargs))
                    entry_armed += 1
                    print(
                        f"ENTRY  R{item.row_number}: {entry.state.value} | "
                        f"entry_intent={entry.entry_intent_id} | {translation.reason}"
                    )
                else:
                    entry_skipped += 1
        ...
        if args.create_entry_intents:
            print(f"Entry intents armed: {entry_armed}  skipped: {entry_skipped}")
            entries = manager.entry_snapshot()
            by_state: dict[str, int] = {}
            for entry in entries:
                by_state[entry.state.value] = by_state.get(entry.state.value, 0) + 1
            print(f"TM Entry active: {len(entries)}  states={by_state or {}}")
    finally:
        manager.stop()
